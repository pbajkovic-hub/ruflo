...
- **Revision-aware upsert (corrected to match the implemented model):** v1 uses a **day-grain** row — one merged row per `(user_id, log_date, source)` fusing all four Oura endpoints (readiness/daily_sleep/sleep/daily_activity). The original `(…, source_record_id)` per-document key is **not** used; the merged row stores `source_record_id` (first non-nil) and `source_updated_at` = **max timestamp across all four endpoints**, with every endpoint's raw JSON accumulated under its own key in `raw_payload` (nothing discarded). Last-write-wins on that max timestamp; a revision to *any* endpoint (sleep-only, activity-only…) re-triggers the day's upsert + a `resolved_daily_metrics` rebuild. Schema enforces this via `UNIQUE(user_id, log_date, source)`; see `OuraClient.merging(...)`.
