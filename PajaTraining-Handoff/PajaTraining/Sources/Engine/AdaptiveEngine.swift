...
public struct ProgressionEngine {
    /// `highConfidence` = the green came from Oura Readiness, not the
    /// low-confidence Apple-Health estimate. A working weight is NEVER bumped
    /// off an estimated green — a wrong estimate would permanently distort load.
    public func next(working: WorkingSet,
        ...
        switch lastOutcome {
        case "hit" where wasGreen && highConfidence:
