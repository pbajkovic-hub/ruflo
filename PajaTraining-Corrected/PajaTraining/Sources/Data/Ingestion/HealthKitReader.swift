...
public struct AppleHealthBodyComposition {
    ...
    // NOTE: skeletal-muscle mass, visceral fat, total-body-water, BMR, InBody
    // score and segmental are NOT standard HealthKit types — they stay nil on
    // the Apple-Health path. (Optional manual supplement is a later add.)
}
...
public final class HealthKitReader {
    ...
    private var readTypes: Set<HKObjectType> {
        ...
        if let t = HKObjectType.quantityType(forIdentifier: .bodyMassIndex) { s.insert(t) }
    ...
    public func bodyComposition(start: Date, end: Date, measuredOn: String) async -> AppleHealthBodyComposition {
        ...
        async let bmi = latestQuantity(.bodyMassIndex, unit: .count(), start: start, end: end)
        let (w, f, l, b) = await (wt, bf, lean, bmi)
    ...
    private func sleepMinutes(start: Date, end: Date) async -> Int? {
        ...
        return await withCheckedContinuation { cont in
            let q = HKSampleQuery(sampleType: st, predicate: pred,
                                  limit: HKObjectQueryNoLimit, sortDescriptors: nil) { _, s, _ in
                // Only the explicit asleep* stages — NOT "everything that
                // isn't inBed" (that wrongly counted `.awake` as sleep).
                // asleepUnspecified subsumes the legacy `.asleep` value.
