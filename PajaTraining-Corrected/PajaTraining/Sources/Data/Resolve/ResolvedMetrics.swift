...
public struct ResolvedMetricsBuilder {
    ...
    // Oura Readiness band — Oura's OWN convention: ≥85 optimal, 70–84 "good /
    // pay attention", <70 = back off. Collapsed to the engine's 3 states so a
    // typical ~71 reads AMBER (progression held), NOT green. (calibratable)
    private func ouraState(_ pct: Int) -> RecoveryState {
        if pct >= 85 { return .green }
        if pct >= 70 { return .amber }
        return .red
    }

    // The Apple-Health estimate is a logistic 0–100 of (HRV/RHR vs baseline) —
    // a DIFFERENT scale from Oura Readiness (z=0 → 50 by construction). Band it
    // symmetrically around the baseline midpoint: clearly-better → green,
    // clearly-worse → red, anything near baseline → amber.
    private func estimateState(_ pct: Int) -> RecoveryState {
        if pct >= 62 { return .green }   // ≳ +0.5 SD better than baseline
        if pct >= 40 { return .amber }
        return .red                      // ≲ −0.6 SD worse than baseline
    }
        ...
        if let b = baseline, let hrv = hrvMs, let rhr = restingHr,
           b.dayCount >= Self.calibrationDays, b.hrvSd > 0, b.rhrSd > 0 {
            ...
            return ResolvedDaily(logDate: date, recoveryPct: pct,
                                 recoveryState: estimateState(pct),
                                 confidence: .low,
                                 sleepScore: sleepScore, sleepTotalMin: sleepTotalMin,
                                 hrvMs: hrvMs, restingHr: restingHr,
